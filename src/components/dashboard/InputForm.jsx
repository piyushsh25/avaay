    ...
    return <div className="form">
        ...
        {state.showButton &&
            <div className="input-icon">
                ...
                <button disabled={(state.name.split(" ").join("").length >= 10 && state.description.split(" ").join("").length >= 20 && state.time >= 1) ? !state.formInValid : state.formInValid} className="button" onClick={() => dispatch({ type: "submit" })}>Submit</button>
            </div>
        }
    </div>
}